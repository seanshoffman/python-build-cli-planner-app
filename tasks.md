...
In the file `src/regular_reminder.py`, create a class named `RegularReminder` with a class method; `__str__(self):`, raising a `NotImplemenetedError` exception
...
Create a class named `ABCMetaReminder` taking `ABCMeta` as its `metaclass` parameter. Add a method, `__str__`, and set the method body to `pass`. Mark the methods with the `@abstractmethod` decorator.